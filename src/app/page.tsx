...
import ProgressBar from "@/components/ProgressBar"; // Importar o novo componente
...
export default function LoginPage() {
  ...
  const handleLogin = async () => {
    ...
    const apiCall = fetch('/api/auth/login', {
      method: 'POST',
      headers: {
    ...
    const minDuration = new Promise(resolve => setTimeout(resolve, 2000)); // Garante 2 segundos de exibição

    try {
      const [res] = await Promise.all([apiCall, minDuration]);
  ...
  return (
    <>
      {isLoading && <ProgressBar duration={2000} />} {/* Passa a duração para a barra de progresso */}
