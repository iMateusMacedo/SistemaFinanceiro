...
const GoalCard: React.FC<GoalCardProps> = ({ title, goalAmount, currentSpent, period, formatCurrency, weekNumberInMonth, currentMonthName }) => {
  ...
  const [animatedPercentageText, setAnimatedPercentageText] = useState(0); // Novo estado para o texto animado

  useEffect(() => {
    setAnimatedWidth(0);
    setAnimatedPercentageText(0);
    ...
    const timer = setTimeout(() => {
      interval = setInterval(() => {
        ...
        const targetTextPercentage = percentageSpent;
        const targetWidthPercentage = Math.min(100, percentageSpent);

        setAnimatedPercentageText(progress * targetTextPercentage);
        setAnimatedWidth(progress * targetWidthPercentage);

        if (currentFrame >= totalFrames) {
          clearInterval(interval);
          setAnimatedPercentageText(targetTextPercentage);
          setAnimatedWidth(targetWidthPercentage);
        }
      }, frameRate);
    }, 100);
    ...
    <div className="bg-slate-700/50 p-6 rounded-xl shadow-lg border border-slate-600 flex flex-col justify-between">
      <div>
        ...
        <div className="w-full bg-gray-700 rounded-full h-6 mb-4 relative overflow-hidden">
          <div 
            className="h-full rounded-full absolute top-0 left-0 bg-gradient-to-r from-green-500 via-yellow-500 to-red-500 transition-all duration-1000 ease-out"
            style={{ width: `${animatedWidth}%` }}
          ></div>
          <div className="absolute inset-0 flex items-center justify-center text-sm font-bold text-white">
            {animatedPercentageText.toFixed(2)}% Gasto
          </div>
        </div>
      </div>
      ...
      {percentageSpent > 50 && percentageSpent <= 90 && (
        <p className="text-center text-yellow-400 font-semibold mt-4">
          Você já consumiu mais da metade dessa meta
        </p>
      )}
      {percentageSpent > 90 && percentageSpent < 100 && (
        <p className="text-center text-orange-400 font-semibold mt-4">
          Cuidado! Você está quase no limite da meta
        </p>
      )}
      {percentageSpent >= 100 && (
  ...
  return (
    <main className="bg-gradient-to-br from-gray-900 to-slate-800 text-gray-100 min-h-screen font-sans p-8">
      <div className="max-w-5xl mx-auto">
        ...
        <p className="text-gray-400 mb-10">Nessa tela, você vai adicionar a quantia máxima que deseja gastar durante o mês. E a partir dela, será calculado as metas diárias, semanais e mensais.</p>
